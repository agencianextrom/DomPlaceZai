...
import { MapPin, Navigation, Store, Star, Clock, ChevronRight, Maximize2, LocateFixed } from 'lucide-react'
...
export function MapStoreLocator({ stores }: MapStoreLocatorProps) {
  ...
  return (
    ...
    >
      ...
      {/* Map area */}
      <div
        className={`relative rounded-2xl overflow-hidden border border-border bg-gradient-to-br from-emerald-50 via-teal-50 to-amber-50 dark:from-emerald-950/30 dark:via-teal-950/20 dark:to-amber-950/20 ${
      ...
      >
        ...
        <div className="absolute top-3 left-3">
          <div className="flex items-center gap-1.5 bg-white/80 dark:bg-black/40 backdrop-blur-sm rounded-lg px-2.5 py-1.5 shadow-sm">
        ...
        <div className="absolute top-3 right-3">
          <div className="flex flex-wrap gap-1 bg-white/80 dark:bg-black/40 backdrop-blur-sm rounded-lg px-2 py-1.5 shadow-sm">
        ...
        {storesWithPositions.map(({ store, position, open }, index) => (
          <motion.button
            ...
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            transition={{ delay: 0.1 + index * 0.05, type: 'spring', stiffness: 400, damping: 15 }}
            ...
            <AnimatePresence>
              {selectedStoreOnMap?.id === store.id && (
                <motion.div
                  ...
                  className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-44 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-border p-2.5 z-50"
                >
                  <div className="flex items-center gap-1.5 mb-1">
                    <Store className="h-3 w-3 text-primary" />
                    <span className="text-xs font-semibold line-clamp-1">{store.name}</span>
                  </div>
                  ...
                  <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-px">
                    <div className="w-2 h-2 bg-white dark:bg-gray-900 rotate-45 border-r border-b border-border" />
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <div className="mt-3 space-y-2">
        {nearbyStores.slice(0, isExpanded ? 6 : 3).map(({ store, open, distance }, index) => (
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={() => handleStoreClick(store)}
            className="w-full flex items-center gap-3 p-3 rounded-xl bg-card border border-border hover:border-primary/20 hover:shadow-sm transition-all text-left group"
          >
            {/* Category dot */}
