  ...
  if (variant === 'card') {
    ...
    if (isVeryLow) {
      return (
        <motion.div
          ...
          className={`mt-1.5 flex items-center gap-1.5 urgency-pulse`}
        >
          <div className="h-1.5 flex-1 rounded-full bg-red-100 dark:bg-red-900/30 overflow-hidden">
            <motion.div
              className="h-full rounded-full bg-red-500 progress-animated"
              style={{ width: `${percentage}%` }}
    ...
    if (isLow) {
      return (
        ...
        >
          <div className="h-1.5 flex-1 rounded-full bg-amber-100 dark:bg-amber-900/30 overflow-hidden">
            <motion.div
              className="h-full rounded-full bg-amber-500 progress-animated"
              style={{ width: `${percentage}%` }}
            />
          </div>
  ...
  if (isVeryLow) {
    return (
      <motion.div
        ...
        className="flex items-center gap-3 mt-3 px-4 py-3 rounded-xl bg-red-50 dark:bg-red-900/15 border border-red-200/60 dark:border-red-800/40 urgency-pulse"
      >
        <motion.div
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="h-2 rounded-full bg-red-200/60 dark:bg-red-800/40 overflow-hidden mt-1.5">
            <motion.div
              className="h-full rounded-full bg-red-500 progress-animated"
              style={{ width: `${percentage}%` }}
              animate={{ opacity: [1, 0.5, 1] }}
        ...
        <motion.span
          ...
          className="text-[9px] font-bold text-white bg-red-500 px-2 py-0.5 rounded-md shrink-0"
        >
          URGENTE
        </motion.span>
      </motion.div>
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="h-2 rounded-full bg-amber-200/60 dark:bg-amber-800/40 overflow-hidden mt-1.5">
            <motion.div
              className="h-full rounded-full bg-amber-500 progress-animated"
              style={{ width: `${percentage}%` }}
            />
    ...
    >
      ...
      <div className="flex-1">
        <div className="h-2 rounded-full bg-emerald-200/60 dark:bg-emerald-800/40 overflow-hidden">
          <motion.div
            className="h-full rounded-full bg-emerald-500 progress-animated"
            style={{ width: `${percentage}%` }}
          />
        </div>
      </div>
